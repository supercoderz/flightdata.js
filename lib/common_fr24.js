...
FR24.REG_BASE = function(reg,page=1,limit=100,token=this.API_TOKEN){
    return 'https://api.flightradar24.com/common/v1/flight/list.json?query='+reg+'&fetchBy=reg&page='+page+'&limit='+limit+'&token='+token
}
FR24.FLT_BASE = function(flt,page=1,limit=100,token=this.API_TOKEN){
...
FR24.login = function(email,password){
	var data = {
		email: email,
		password: password,
    }
	...
	return common.post(this.LOGIN_URL,data,function(res){
        this.API_TOKEN = res.userData.subscriptionKey
    },function(err){
        console.log('Error logging in to flightradar24')
    },options)
}
